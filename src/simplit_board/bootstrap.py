...
def _apt_install(*packages: str) -> None:
    ...
    r = subprocess.run([*_sudo(), "apt-get", "install", "-y", "--no-install-recommends", *packages],
...
def _make_state_dir() -> None:
    ...
    if user:
        cmds.append([*_sudo(), "chown", "-R", f"{user}:{user}", d])
    cmds.append([*_sudo(), "chmod", "750", d])
    for cmd in cmds:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
        if r.returncode != 0:
            raise RuntimeError(f"{' '.join(cmd)} failed: {r.stderr[-200:]}")
...
PREREQS: List[Prerequisite] = [
    Prerequisite("os-essentials", 10, _essentials_present,
                 lambda: _apt_install("ca-certificates", "curl", "procps", "util-linux")),

    Prerequisite("jre-21", 20, _java_ok,
                 lambda: _apt_install("openjdk-21-jre-headless")),

    Prerequisite("python3", 30, lambda: _has("python3"),
                 lambda: _apt_install("python3", "python3-venv")),


    Prerequisite("container-runtime", 40, lambda: _has("docker"),
                 lambda: _apt_install("docker.io"), required=False),
