...
class Supervisor:
    ...
    def __init__(self, jar_dir: Path, java_bin: str='java', baked_jar: Optional[str]=None, device_id: Optional[str]=None, device_secret: Optional[str]=None, token_url: Optional[str]=None, presence_url: Optional[str]=None, register_url: Optional[str]=None, delivery_pubkey: Optional[str]=None):
    ...
    def _launch(self, version: str, extra: list[str], grace_seconds: float=35.0) -> str:
        ...
        dpub = self.delivery_pubkey or os.environ.get('SIMPLIT_DELIVERY_PUBKEY') or os.environ.get('SIMPLIT_DELIVERY_PUBLIC_KEY')
        if dpub:
        ...
        self.proc = subprocess.Popen(['bash', str(wrapper)], stdout=log, stderr=subprocess.STDOUT, env=env, start_new_session=True)
