...
def register(email: str | None, password: str | None, subdivision: str | None, mfa_code: str | None) -> None:
    ...
    if not secret:
        ...
        if anchor:
            ...
            click.echo('trust anchor: enrollment returned no signing key — set SIMPLIT_DELIVERY_PUBKEY before `up`, or ask an operator to configure enrollment.signing-pubkey.', err=True)
...
def up() -> None:
    ...
    if delivery_key is None and control_key is None:
        click.echo('no trusted signing key (SIMPLIT_DELIVERY_PUBKEY / SIMPLIT_CONTROL_PUBKEY) — refusing to accept unsigned deploys.', err=True)
        sys.exit(2)
    supervisor = Supervisor(cfg.jar_dir, device_id=st.name, device_secret=secret, token_url=cfg.token_url, presence_url=cfg.presence_ws_url, register_url=cfg.register_url, delivery_pubkey=delivery_pub_b64)

    def handle_artifact(manifest: dict, data: bytes) -> dict:
        click.echo(f'[deploy] verified {len(data)} bytes ({manifest.get('service')} v={manifest.get('version')}) — installing the board service from the received bytes …')
    ...
    def handle_push(frame: dict) -> dict:
        ...
        return {'boardId': st.name, 'status': 'deployed', 'detail': detail}
    tokens = TokenProvider(cfg.token_url, st.client_id or st.name, secret)
    presence = PresenceClient(cfg.presence_ws, tokens, st.name, handler=handle_push if control_key else None, log=click.echo, yield_after_deploy=True, artifact_handler=handle_artifact, delivery_key=delivery_key)
    click.echo(f"board '{st.name}' is online — no software installed yet.")
    click.echo('waiting for the operator to push the board service from the console (Updates ▸ push to this device)…  Ctrl-C to stop')
    try:
    ...
    if ephemeral:
        click.echo(f"\nboard '{st.name}' installed + running — it now owns the presence session and receives its own updates directly (re-push goes straight to the board → verify → self-restart). The agent's job is done; exiting. The board is self-managed from here.")
        return
