...
class PresenceClient:
    ...
    def __init__(self, ws_url: str, token_provider, device_id: str, handler: Optional[JobHandler]=None, log=print, yield_after_deploy: bool=False, artifact_handler: Optional[ArtifactHandler]=None, delivery_key=None):
        self._ws_url = ws_url
    ...
    def _on_device_job(self, ws, frame: dict) -> None:
        ...
        try:
            result = self._handler(frame) if self._handler else {'boardId': self._device_id, 'status': 'rejected', 'detail': 'no job handler'}
        except Exception as e:
    ...
    def _on_deploy_start(self, frame: dict) -> None:
        ...
        manifest = {k: frame.get(k) for k in ('service', 'version', 'size', 'sha256', 'chunkCount', 'chunkBytes', 'sig')}
        authentic = False
        ...
        self._log(f'[deploy] {deploy_id} start: {manifest.get('service')} v={manifest.get('version')} {manifest.get('size')}B in {manifest.get('chunkCount')} chunks — signature {('VERIFIED' if authentic else 'REJECTED')}')
    ...
    def _assemble_and_deploy(self, st: Optional[dict]) -> dict:
        ...
        if len(data) != int(m['size']):
            return {'boardId': self._device_id, 'status': 'failed', 'detail': f'size mismatch: got {len(data)}, expected {m['size']}'}
